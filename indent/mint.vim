...
let s:languages = [
      \   { 'name': 'css', 'pairs': ['style [a-zA-Z0-9:-]* {', '}'] },
      \   { 'name': 'javascript', 'pairs': ['`', '`'] },
      \   { 'name': 'html', 'pairs': ['<[^>]*>', '<\/[a-zA-Z]*>'] },
      \ ]
...
let s:go_indent = s:get_indentexpr('go')

let b:did_indent = 1

setlocal indentexpr=MintIndent()

if exists('*MintIndent')
  finish
endif
...
function! MintIndent()
  ...
  if exists('l:indent')
    ...
    " Couldn't find language, fall back to golang
    execute 'let indent = ' . s:go_indent
  endif
